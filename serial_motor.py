...
# 如果没有安装pyserial，需要先在树莓派上执行:
# sudo apt-get update
# sudo apt-get install python3-pip
# pip3 install pyserial

###############################################################################
# 1. 多项式拟合相关配置（保持你的原有代码）
###############################################################################
driverPUL = 12  
driverDIR = 18  
Steps_per_revolution = 790

# 示例：将你的 500 组 (激励, speed) 数据粘贴在此
        ...
        while True:
            ...
            if ser.in_waiting > 0:
                ...
                if line:
                    try:
                        # 尝试将读取到的字符串解析为浮点数
                        desired_excitation = float(line)
                        print(f"收到目标激励: {desired_excitation}")
                        ...
                        print(f"拟合得到的电机转速为: {forward_speed} steps/s")

                        # 3.3 电机演示动作：先正转 90 度，再以线性减速反转 90 度
                        ...
                        reverse_start_speed = 300  # 反转开始时的较快速度，可自行调整
                        reverse_end_speed   = 100  # 反转结束时的较慢速度，可自行调整
                        ...
                        print(f"串口读取到的内容无法解析成float: {line}")
